...
def main():
    ...
    analytical = an_solution.constant_charge_single_energy(-80*1, 4, 0.125, 80)
...
#    plt.rc('font',**font)
#    loc = (3*N[0]+N[1])/4
#    tex_loc = numpy.array((loc,N[0]*error[0]/loc))
...
#    fig.savefig('regression_tests/figs/error_energy_neumann_surface.pdf',dpi=80,format='pdf')
#
#    fig = plt.figure(figsize=(3,2), dpi=80)
#    ax = fig.add_subplot(111)
...
#    ax.loglog(N, asymp,c='k',marker='None',ls=':', lw=0.8, label=None)
#    loc = (3*N[0]+N[1])/4
#    tex_loc = numpy.array((loc, loc*numpy.log(loc)*total_time[0]/(N[0]*numpy.log(N[0]))))
#    tex_angle = math.atan2(numpy.log(abs(asymp[-1]-asymp[0])),numpy.log(abs(N[-1]-N[0])))*180/math.pi
...
#    ax.set_xlabel('Number of elements', fontsize=10)
#    fig.subplots_adjust(left=0.185, bottom=0.21, right=0.965, top=0.95)
#    fig.savefig('regression_tests/figs/total_time_neumann_surface.pdf',dpi=80,format='pdf')
#
#    fig = plt.figure(figsize=(3,2), dpi=80)
#    ax = fig.add_subplot(111)
...
#    ax.set_xlabel('Number of elements', fontsize=10)
#    fig.subplots_adjust(left=0.185, bottom=0.21, right=0.965, top=0.95)
#    fig.savefig('regression_tests/figs/iterations_neumann_surface.pdf',dpi=80,format='pdf')
