...
def report_e():
    print('2 Spheres test could not run using GPU because pycuda were not found. CPU were used instead.')
def report_g():
    print('2 Spheres test had run using GPU.')
def report_c():
    print('2 Spheres test had run using CPU.')
...
def test_two_sphere_iterations(arc):
    results = get_results(arc)
    with open('two_sphere.pickle', 'rb') as f:
        base_results = pickle.load(f)
