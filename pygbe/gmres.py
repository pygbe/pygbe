...
from pygbe.projection import project
...
def gmres_mgs(surf_array, field_array, X, b_clean, param, ind0, timing, kernel):
    """
    GMRES solver.
    ...
    # import.
    xtype = upcast(X.dtype, b_clean.dtype)
...
def gmres_sigma(surf, ss, X, b, param, ind0, timing, kernel):
    """
    GMRES solver for apparent surface charge.
    ...
    # Prep for method
    #aux = gmres_dot(X, surf_array, field_array, ind0, param, timing, kernel)
    dummy, aux = project(numpy.zeros(len(X)), X, 1, surf, surf,
    ...
    for outer in range(max_outer):
        ...
        for inner in range(max_inner):
            #New search direction
            v= V[inner+1, :]
            #v[:] = gmres_dot(vs[-1], surf_array, field_array, ind0, param, timing, kernel)
            dummy, v[:] = project(numpy.zeros(len(vs[-1])), vs[-1], 1, surf, surf,
                            0., 0., 1, ss, param, ind0, timing, kernel)
            vs.append(v)
