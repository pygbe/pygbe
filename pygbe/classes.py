...
class surfaces():
    def __init__(self):
        ...
        self.xcSort = []  # sorted x component box centers by M2P_list array
        self.ycSort = []  # sorted y component box centers by M2P_list array
        self.zcSort = []  # sorted z component box centers by M2P_list array
        self.AreaSort = []  # sorted array of areas
        self.sglInt_intSort = [
        ]  # sorted array of singular integrals for V for internal equation
        self.sglInt_extSort = [
        ]  # sorted array of singular integrals for V for external equation
        ...
        self.Precond = [
        ]  # Sparse representation of preconditioner for self interaction block
        ...
        self.surf_type = 0  # Surface type: internal_cavity (=0),
                            #stern or dielectric_interface (=1)
        self.phi0 = [
        ]  # Known surface potential (dirichlet) or derivative of potential (neumann)
...
def getGaussPoints(y, triangle, n):
    ...
    if n == 7:
        for i in range(N):
            ...
            xi[n * i + 1, :] = numpy.dot(M, numpy.array(
                [.797426985353087, .101286507323456, .101286507323456]))
            xi[n * i + 2, :] = numpy.dot(M, numpy.array(
                [.101286507323456, .797426985353087, .101286507323456]))
            xi[n * i + 3, :] = numpy.dot(M, numpy.array(
                [.101286507323456, .101286507323456, .797426985353087]))
            xi[n * i + 4, :] = numpy.dot(M, numpy.array(
                [.059715871789770, .470142064105115, .470142064105115]))
            xi[n * i + 5, :] = numpy.dot(M, numpy.array(
                [.470142064105115, .059715871789770, .470142064105115]))
            xi[n * i + 6, :] = numpy.dot(M, numpy.array(
                [.470142064105115, .470142064105115, .059715871789770]))
