...
def AI(y, x, same):
    ...
    Lu     = numpy.array([L[0]/numpy.norm(L[0]), L[1]/numpy.norm(L[1]), L[2]/numpy.norm(L[2])]) # Unit vectors parallel to L0, L1 and L2
    ...
    tu = numpy.array([numpy.cross(normal,Lu[0]),numpy.cross(normal,Lu[1]),numpy.cross(normal,Lu[2])]) # Unit vector in triangle plane normal to L0, L1 and L2

    etha = numpy.dot(x-y[0], normal)
    rho = numpy.array([numpy.norm(x-y[0]), numpy.norm(x-y[1]), numpy.norm(x-y[2])])

    pp = numpy.zeros((3))
    pp = [numpy.dot(x-y[0],Lu[0]), numpy.dot(x-y[0],Lu[1]), numpy.dot(x-y[0],Lu[2])] # Distance x to y0 projected in L0, L1, and L2

    p = numpy.zeros((3,3))
    p[0] = [numpy.dot(x-y[0],Lu[0]), numpy.dot(x-y[0],Lu[1]), numpy.dot(x-y[0],Lu[2])] # Distance x to y0 projected in L0, L1, and L2
    p[1] = [numpy.dot(x-y[1],Lu[0]), numpy.dot(x-y[1],Lu[1]), numpy.dot(x-y[1],Lu[2])] # Distance x to y1 projected in L0, L1, and L2
    p[2] = [numpy.dot(x-y[2],Lu[0]), numpy.dot(x-y[2],Lu[1]), numpy.dot(x-y[2],Lu[2])] # Distance x to y2 projected in L0, L1, and L2
    ...
    rhop1 = numpy.array([numpy.norm(x-y[1]), numpy.norm(x-y[2]), numpy.norm(x-y[0])])
...
def AI_arr(DorN,y, x, same, E_hat):
    ...
    chi = numpy.log(numpy.transpose(numpy.array([p00,p11,p22])) + rho) - numpy.log(pp1 + rhop1)
    ...
    THETA = numpy.where(etha<1e-10, 0.5*numpy.sum(gamma,axis=1)-theta0, 0.5*numpy.sum(gamma,axis=1)+theta0)
...
def getGaussPoints(y,triangle, n):
    ...
    if n==7:
        for i in range(N):
            ...
            xi[n*i+1,:] = numpy.dot(M, numpy.array([.79742699,.10128651,.10128651]))
            xi[n*i+2,:] = numpy.dot(M, numpy.array([.10128651,.79742699,.10128651]))
            xi[n*i+3,:] = numpy.dot(M, numpy.array([.10128651,.10128651,.79742699]))
            xi[n*i+4,:] = numpy.dot(M, numpy.array([.05971587,.47014206,.47014206]))
            xi[n*i+5,:] = numpy.dot(M, numpy.array([.47014206,.05971587,.47014206]))
            xi[n*i+6,:] = numpy.dot(M, numpy.array([.47014206,.47014206,.05971587]))
