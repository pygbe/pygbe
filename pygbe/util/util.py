...
def get_gamma(p1, p2, qet, cr1, cr2, zn, zd):
    ...
    if (p1 < 0. and p2 > 0.):
        ...
        if qet < 0.:
            cond = (cr1 > 0. and cr2 < 0. and zn > 0. and zd < 0.) or (
                cr1 < 0. and cr2 > 0. and zn > 0. and zd < 0.) or (
                    cr1 < 0. and cr2 < 0. and zn > 0.)
            if cond:
...
def calculate_gamma(p, q, rho, etha):
    ...
    gamma = numpy.zeros(3)
    gamma[0] = get_gamma(p[0, 0], p[1, 0], 2 * q[0] * etha, cr11, cr12, zn1,
                         zd1)
    gamma[1] = get_gamma(p[1, 1], p[2, 1], 2 * q[1] * etha, cr22, cr23, zn2,
                         zd2)
    gamma[2] = get_gamma(p[2, 2], p[0, 2], 2 * q[2] * etha, cr33, cr31, zn3,
                         zd3)
