...
def get_gamma(p1, p2, qet, cr1, cr2, zn, zd):
    ...
    cond_aux1 = numpy.logical_and(cr1 > 0., numpy.logical_and(cr2 < 0.,
                                                              zd < 0.))
    cond_aux2 = numpy.logical_and(cr1 < 0., numpy.logical_and(cr2 > 0.,
                                                              zd < 0.))
    cond1 = numpy.logical_or(
        numpy.logical_and(cond_aux1, zn > 0.), numpy.logical_or(
            numpy.logical_and(cond_aux2, zn > 0.), numpy.logical_and(
                cr1 < 0., numpy.logical_and(cr2 < 0., zn > 0.))))
    cond2 = numpy.logical_or(
        numpy.logical_and(cond_aux1, zn < 0.), numpy.logical_or(
            numpy.logical_and(cond_aux2, zn < 0.), numpy.logical_and(
                cr1 < 0., numpy.logical_and(cr2 < 0., zn < 0.))))
    ...
    gm = numpy.where(
        numpy.logical_and(C3, numpy.logical_and(
            logical_not(C1), logical_not(C2))), numpy.zeros(len(gm)), gm)
...
def calculate_gamma(p00, p11, p22, p10, p21, p02, q, rho, etha):
    ...
    zn1 = 2 * q[:, 0] * etha * (
        p10 * rho[:, 1] * cr11 - p00 * rho[:, 0] * cr12)
    zd1 = cr11 * cr12 + (2 * q[:, 0] *
                         etha)**2 * p00 * rho[:, 0] * p10 * rho[:, 1]

    cr22 = q[:, 1]**2 * rho[:, 1]**2 - p11**2 * etha**2
    cr23 = q[:, 1]**2 * rho[:, 2]**2 - p21**2 * etha**2
    zn2 = 2 * q[:, 1] * etha * (
        p21 * rho[:, 2] * cr22 - p11 * rho[:, 1] * cr23)
    zd2 = cr22 * cr23 + (2 * q[:, 1] *
                         etha)**2 * p11 * rho[:, 1] * p21 * rho[:, 2]

    cr33 = q[:, 2]**2 * rho[:, 2]**2 - p22**2 * etha**2
    cr31 = q[:, 2]**2 * rho[:, 0]**2 - p02**2 * etha**2
    zn3 = 2 * q[:, 2] * etha * (
        p02 * rho[:, 0] * cr33 - p22 * rho[:, 2] * cr31)
    zd3 = cr33 * cr31 + (2 * q[:, 2] *
                         etha)**2 * p22 * rho[:, 2] * p02 * rho[:, 0]
...
def test_pos(aQ, bQ, cQ, q, p00, same):
    ...
    cond = numpy.logical_or(same == 1, numpy.logical_and(
        shc[:, 0] > 1e-15, numpy.logical_and(shc[:, 1] > 1e-15,
                                             shc[:, 2] > 1e-15)))
    position = numpy.where(cond, numpy.ones(len(p00)) * 2 * pi,
                           numpy.zeros(len(p00)))

    # Condition if on any edge   
    # Edge 2
    cond = numpy.logical_and(
        numpy.abs(shc[:, 0]) < 1e-12, numpy.logical_and(shc[:, 1] > 0.,
                                                        shc[:, 2] > 0.))
    position = numpy.where(cond, numpy.ones(len(p00)) * pi, position)
    # Edge 3
    cond = numpy.logical_and(
        numpy.abs(shc[:, 1]) < 1e-12, numpy.logical_and(shc[:, 2] > 0.,
                                                        shc[:, 2] < 1.))
    position = numpy.where(cond, numpy.ones(len(p00)) * pi, position)
    # Edge 1
    cond = numpy.logical_and(
        numpy.abs(shc[:, 2]) < 1e-12, numpy.logical_and(shc[:, 1] > 0.,
                                                        shc[:, 1] < 1.))
    position = numpy.where(cond, numpy.ones(len(p00)) * pi, position)
